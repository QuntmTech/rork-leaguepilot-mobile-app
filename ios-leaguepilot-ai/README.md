...
While signed in, the app keeps a native PocketBase realtime subscription for connection, job, snapshot, recommendation, and report changes. Transient stream failures reconnect with capped exponential backoff and jitter; a healthy subscription resets that backoff. A confirmed `401` stops realtime work and returns to authentication without a retry loop. A `403` is checked with token refresh and, when the session remains valid, shown as a subscription-permission error instead of deleting the session. Those events invalidate the relevant SwiftUI tasks, so a review completed on the web is reflected in mobile without an app restart. APNs device registration and durable alert read/unread state are intentionally not implemented because the backend support for them is not yet available.
