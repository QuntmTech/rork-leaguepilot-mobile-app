...
function ConnectedHome({ navigate, runAnalysis, decisions, lastSync }: { navigate: (screen: Screen, id?: string) => void; runAnalysis: () => void; decisions: Record<string, Decision>; lastSync: string }) {
  ...
  return <div className="v2-screen v2-with-nav"><AppHeader title="Game plan" eyebrow="WEEK 9 • THURSDAY" action={<div className="v2-head-actions"><button onClick={() => navigate("activity")}><Bell /><i /></button><button className="v2-avatar" onClick={() => navigate("settings")}>CW</button></div>} /><main className="v2-scroll v2-home"><DemoLabel /><MatchupDeck navigate={navigate} /><section className="v2-readiness"><div className="v2-readiness-ring" style={{ "--score": "92%" } as React.CSSProperties}><span><strong>92</strong><small>READY</small></span></div><div><p className="v2-overline">WEEKLY READINESS</p><h2>Two decisions could sharpen your edge.</h2><span>Lineup verified • Waiver market scanned • Report ready</span></div></section><button className="v2-analysis-button" onClick={runAnalysis}><span><Zap /></span><p><strong>Run fresh analysis</strong><small>Sync → lineup → waivers → trades → report</small></p><ArrowRight /></button><div className="v2-section-title"><div><p>COMMAND QUEUE</p><h2>{openDecisions.length || 0} decisions waiting</h2></div><button onClick={() => navigate("recommendations")}>See all</button></div><div className="v2-home-recs">{openDecisions.slice(0, 2).map((rec) => <RecommendationCard key={rec.id} rec={rec} open={() => navigate("recommendation", rec.id)} />)}{openDecisions.length === 0 && <div className="v2-empty"><CheckCircle2 /><p><strong>Decision queue cleared</strong><span>You reviewed every current recommendation.</span></p></div>}</div><div className="v2-intel-grid"><button onClick={() => navigate("league")}><span><LayoutGrid /></span><p><small>MY LEAGUE</small><strong>Roster & matchup</strong></p><ChevronRight /></button><button onClick={() => navigate("report")}><span><FileText /></span><p><small>LATEST REPORT</small><strong>Week 9 intelligence</strong></p><ChevronRight /></button></div><div className="v2-data-note"><Info /><p><strong>Partial ESPN projection data</strong><span>Unavailable values stay unavailable. Current recommendations use only verified inputs.</span></p></div><p className="v2-last-sync">Last prototype sync: {lastSync}</p></main><BottomNav screen="home" navigate={navigate} /></div>;
}
...
export function LeaguePilotV2() {
  ...
  return <main className="v2-shell"><aside className={`v2-control ${menuOpen ? "open" : ""}`}><header><Brand /><button onClick={() => setMenuOpen(false)} aria-label="Close preview menu"><X /></button></header><div className="v2-control-intro"><p>V2 • NATIVE SWIFTUI FOUNDATION</p><h1>A real weekly decision workflow.</h1><span>Built from the Rork project’s SessionStore, HomeView, ConnectESPNView and PocketBase service flow—then expanded into the complete product vision.</span></div><button className="v2-journey" onClick={() => { signOut(); setMenuOpen(false); }}><KeyRound /><span><strong>Start from Sign In</strong><small>Complete first-time journey</small></span><ArrowRight /></button><nav><h2>QUICK JUMP</h2>{jumpItems.map(({ id, label, icon: Icon }) => <button key={id} className={screen === id ? "active" : ""} onClick={() => { if (!authenticated) setAuthenticated(true); if (id !== "home" && !connected) setConnected(true); if (id === "analysis") { setAnalysisStep(0); setAnalysisRunning(true); } navigate(id); }}><Icon /><span>{label}</span>{screen === id && <Check />}</button>)}</nav><section className="v2-test-states"><h2>TEST RECOVERY</h2><div><button className={network === "offline" ? "active" : ""} onClick={() => setNetwork(network === "offline" ? "online" : "offline")}><WifiOff />Offline</button><button className={network === "error" ? "active" : ""} onClick={() => setNetwork(network === "error" ? "online" : "error")}><CircleAlert />ESPN error</button></div></section><footer><span><ShieldCheck />Data-honest prototype</span><span>SwiftUI base • 393 × 852</span></footer></aside><section className="v2-stage"><button className="v2-mobile-menu" onClick={() => setMenuOpen(true)}><Menu />Preview menu</button><div className="v2-stage-meta"><span><i />LIVE V2 PROTOTYPE</span><span>Native iOS structure • 393 × 852</span></div><div className="v2-phone"><StatusBar /><div className="v2-viewport">{hydrated ? rendered : <div className="v2-loading"><LoaderCircle className="spin" />Loading command center…</div>}<NetworkOverlay mode={network} close={() => setNetwork("online")} /></div><div className="v2-home-bar" /></div><p className="v2-stage-note"><Info />Functional prototype based on the Rork SwiftUI app. Interactions persist locally; ESPN/PocketBase actions are safely simulated.</p></section><Toaster richColors position="top-center" /></main>;
}
